...
public class XYscope {
     ...
     *
     * @param theParent PApplet to apply to, typically 'this'
     * @param sampleR   Sample rate for soundcard (44100, 48000, 96000, 192000).
     */
     ...
     * @param theParent     PApplet to apply to, typically 'this'
     * @param xyMixer       Name of custom sound mixer to use for XY.
     * @param sampleRateVal Sample rate for soundcard (44100, 48000, 96000, 192000).
     * @param bufferSizeVal Size of buffer/latency for cpu/soundcard (128, 256, 512, 1024, 2048).
     */
     ...
     *
     * @param zMixer  Name of custom sound mixer to use for Z.
     * @param sampleR Sample rate for soundcard (44100, 48000, 192000).
     */
     ...
     */
    public float[] zRange() {
        return new float[]{zaxisMin, zaxisMax};
    }
     ...
     *
     * @param vrot int for degrees of rotation, 90 or -90
     */
     ...
     *
     * @param vw   int for width of canvas, default is 330
     * @param vh   int for height of canvas, default is 410
     * @param vamp float for initial amplitude adjustment of signal to screen (0.0 - 1.0), default is .6
     * @param vrot int for degrees of rotation, 90 or -90, default is 0
     */
     ...
     *
     * @param vectrexAmpVal float for amplitude difference (0.0 - 1.0), default is .82
     */
     ...
     *
     * @param inR  String for name of audio channel for Red
     * @param inBG String for name of audio channel for Blue/Green
     */
     ...
     *
     * @param newLaserLPFVal float between 0.1 - 20000.0
     */
     ...
     *
     * @param minR value between 0.0 - 255.0
     * @param minG value between 0.0 - 255.0
     * @param minB value between 0.0 - 255.0
     */
     ...
     *
     * @param minPV PVector with 3 values between 0.0 - 255.0
     */
     ...
     *
     * @param wbR value between 0.0 - 255.0
     * @param wbG value between 0.0 - 255.0
     * @param wbB value between 0.0 - 255.0
     */
     ...
     *
     * @param wbPV PVector with 3 values between 0.0 - 255.0
     */
     ...
     *
     * @param newDash PVector with 3 values
     */
     ...
     *
     * @param r float from 0.0 – 255.0
     * @param g float from 0.0 – 255.0
     * @param b float from 0.0 – 255.0
     */
     ...
     *
     * @param rgb PVector with 3 values, from 0.0 – 255.0
     */
     ...
     *
     * @param newFreqR float
     * @param newFreqG float
     * @param newFreqB float
     */
     ...
     *
     * @param newAmpX value between 0.0 - 1.0
     * @param newAmpY value between 0.0 - 1.0
     */
     ...
     * @param newAmpX value between 0.0 - 1.0
     * @param newAmpY value between 0.0 - 1.0
     * @param newAmpZ value between 0.0 - 1.0
     */
     ...
     *
     * @param newFreqX float
     * @param newFreqY float
     */
     ...
     *
     * @param panXVal float - pan for x/left channel
     * @param panYVal float - pan for y/right channel
     */
...
//		waveSize(1024);
    ...
    public void buildWaves(int bwm) {
        if (bwm == 0) { // waveform gen v4 mar 2020... 23
            if (shapes.size() > 0) {
                ...
                double tot = 0.0;
                ArrayList<PVector> wave_col = new ArrayList<>();
                for (XYShape shape : shapes) {
                    double shapeDist = shape.getDistance();
                    for (int j = 0; j < shape.size() - 1; j++) { // what about point?
            ...
            if (shapes.size() > 0) {
                if (shapes.totalSize() < waveSizeValOG) {
                    ...
                    if (waveSize() == shapes.totalSize()) {
                        for (XYShape shape : shapes) {
                            for (PVector pVector : shape) {
                                shapePreX[SID] = map(pVector.x, 0f, 1f, -1f, 1f);
                                shapePreY[SID] = map(pVector.y, 0f, 1f, 1f, -1f);
                                SID++;
                            }
                        }
                    }
        ...
        if (bwm == -1 || bwm == -2) {
            ...
            if (useEase) {
                ...
                if (useZ) {
                    System.arraycopy(shapePreZ, 0, shapeZ, 0, shapePreZ.length);
                }
            }
    ...
    private void buildColorWave(XYWavetable tableTemp, String RGBval, int dashTemp) {
        ...
        for (int i = 0; i < shapeTemp.size(); i++) {
            ...
            if (Objects.equals(RGBval, "x")) {
                tfr[i] = tc.x;
            } else if (Objects.equals(RGBval, "y")) {
                tfr[i] = tc.y;
            } else if (Objects.equals(RGBval, "z")) {
                tfr[i] = tc.z;
            }
    ...
    public void buildX(float[] newWave) {
        ...
        if (useEase) {
            ...
            System.arraycopy(shapePreX, 0, shapeX, 0, shapePreX.length);
        }
    }
    ...
    public void buildY(float[] newWave) {
        ...
        if (useEase) {
            ...
            System.arraycopy(shapePreY, 0, shapeY, 0, shapePreY.length);
        }
    }
    ...
    public void drawPath(float r, float g, float b) {
        ...
        for (XYShape shape : shapes) {
            for (PVector pVector : shape) {
                float x = map(pVector.x, 0f, 1f, 0f, xyWidth);
                float y = map(pVector.y, 0f, 1f, 0f, xyHeight);
                myParent.vertex(x, y);
            }
        }
    ...
    public void drawXY(float r, float g, float b) {
        ...
        myParent.stroke(r, g, b);
        myParent.pushMatrix();
        myParent.translate(xyWidth / 2f, xyHeight / 2f);
        myParent.beginShape();
        AudioOutput tempXY;
        if (useMix) {
            tempXY = mixXY;
     ...
     *
     * @param newED int for facets of ellipse
     */
     ...
     * Set detail (number of points) for drawing an ellipse.
     *
     * @param newED float for facets of ellipse (rounded)
     */
     ...
     *
     * @param newSteps float for segments multiplier (rounded)
     */
    ...
    // non-params drawing
    public void point() {
        line(xyWidth / 2f, xyHeight / 2f, xyWidth / 2f + 1, xyHeight / 2f + 1);
    }
     ...
     * @param e float - width + height of square
    ...
    // non-params drawing
    public void circle() {
        vertexEllipse(xyWidth / 2f, xyHeight / 2f, xyHeight, xyHeight);
    }
     ...
     * @param e float - width + height of circle
    ...
    public void lissajous() {
        lissajous(xyWidth / 2f, xyHeight / 2f, xyHeight / 2f, 1, 2, 0, 180);
    }
    ...
    public void box() {
        box(xyHeight / 2f, xyHeight / 2f, xyHeight / 2f);
    }
     ...
     *
     * @param rxt float - size of box in x-axis
     * @param ryt float - size of box in y-axis
     * @param rzt float - size of box in z-axis
     * @see <a href="https://processing.org/reference/box_.html">Processing
     * Reference » box()</a>
     */
    ...
    // non-param drawing
    public void sphere() {
        ellipsoid(xyHeight / 3f, xyHeight / 3f, xyHeight / 3f, dx, dy);
    }
     ...
     *
     * @param rs  float - size of sphere
     * @param dxy int - number of horzontal + vertical vertices
     * @see <a href="https://processing.org/reference/sphere_.html">Processing
     * Reference » sphere()</a>
     */
     ...
     *
     * @param rs float - size of sphere
     * @param dx int - number of horzontal vertices
     * @param dy int - number of vertical vertices
     * @see <a href="https://processing.org/reference/sphere_.html">Processing
     * Reference » sphere()</a>
     */
     ...
     * Draw ellipsoid, expects ellipsoid(rx, ry, rz, verticesCount).
     *
     * @param rxt float - size in x-axis
     * @param ryt float - size in y-axis
     * @param rzt float - size in z-axis
     * @param dx  int - number of horzontal vertices
     * @param dy  int - number of vertical vertices
     * @see <a href="https://processing.org/reference/sphere_.html">Processing
     * Reference » sphere()</a>
    ...
    public void ellipsoid(float rxt, float ryt, float rzt, int dx, int dy) {
        ...
        for (int i = 0; i < numPointsH; i++) {  // For all points from top to bottom
            if (numPointsH_2pi % 2 == 0 && i == numPointsH - 1) {  // If the numPointsH_2pi is odd and it is at the last pt
    ...
    // non-param drawing
    public void torus() {
        torus(xyHeight / 4f, xyHeight / 6f, dx, dy);
    }
...
//		}
     ...
     *
     * @param fontName String of Hershey Font name
     */
    public void textFont(String fontName) {
        if (contains(fonts, fontName)) {
            ...
            StringBuilder hershey_font_string = new StringBuilder();

            for (String s : hershey_font_org) {
                String line = s.trim();
                if (line.charAt(0) >= 48 && line.charAt(0) <= 57)
                    hershey_font_string.append(line).append("\n");
                else {
                    hershey_font_string = new StringBuilder(hershey_font_string.substring(0, hershey_font_string.length() - 1) + line + "\n");
                }
            }
            hershey_font = hershey_font_string.toString().split("\n");
        }
    }
     ...
     *
     * @param fontSize Float - size of text
     */
     ...
     *
     * @param leading Float - 	the size in pixels for spacing between lines
     */
    ...
    // non-param drawing
    public void text() {
        text("XYscope", xyWidth / 2f, xyHeight / 2f);
    }
     ...
     * @param x float - horizontal position of text
     * @param y float - vertical position of text
     */
    public void text(String s, float x, float y) {
        String[] parts = splitTokens(s, "\n\r");

        switch (textAlignY) {
            case 101 -> y += hfactor * 12;
            case 102 -> y -= hfactor * 21 * parts.length;
            default -> {
                if (parts.length > 1) {
...
//		PVector ta = textOffset(s, x, y);
        ...
        switch (textAlignX) {
            case 3 -> x -= textWidth(s) / 2;
            case 39 -> x -= textWidth(s);
        }

        switch (textAlignY) {
            case 101 -> y += hfactor * 12;
            case 102 -> y -= hfactor * 12;
        }
     ...
     *
     * @param c char - character to measure width of
     * @return float
     */
     ...
     *
     * @param s String - text to measure width of
     * @return float
     */
    public float textWidth(String s) {
        ...
        float offxMax = 0;
        for (String part : parts) {
            float offxTemp = textWidthParse(part);
            if (offxTemp > offxMax) {
                offxMax = offxTemp;
            }
        }
    ...
    public PVector[][] textPaths(String s, float x, float y) {
        ...
        switch (textAlignY) {
            case 101 -> y += hfactor * 12;
            case 102 -> y -= hfactor * 21 * parts.length;
            default -> y -= hfactor * 21 * parts.length / 2;
        }

        ArrayList<ArrayList<PVector>> cooords = new ArrayList<>(s.length());
        float yOffset = y;
        for (String part : parts) {
            textPathsParse(part, x, yOffset, cooords);
            yOffset += hfactor * hheight + hleading;
        }
    ...
    void textPathsParse(String s, float x, float y, ArrayList<ArrayList<PVector>> coords) {
        ...
        for (int k = 0; k < s.length(); k++) {
            ...
            String[] h_vertices = h.substring(start_col + 5).replaceAll(" R", " ").split(" ");

            for (String hVertex : h_vertices) {
                ArrayList<PVector> coord = new ArrayList<>(hVertex.length());
                for (int j = 2; j < hVertex.length(); j += 2) {
                    float hx0 = hershey2coord(hVertex.charAt(j - 2)) * hfactor;
                    float hy0 = hershey2coord(hVertex.charAt(j - 1)) * hfactor;
                    coord.add(new PVector(offx + hx0, offy + hy0));
                    float hx1 = hershey2coord(hVertex.charAt(j)) * hfactor;
                    float hy1 = hershey2coord(hVertex.charAt(j + 1)) * hfactor;
    ...
    public static class XYShapeList extends ArrayList<XYShape> {
        ...
        public ArrayList<PVector> getPoints() {
            ArrayList<PVector> gp = new ArrayList<>();
            for (XYShape shape : this) {
                gp.addAll(shape);
            }
            return gp;
    ...
    public static class XYShape extends ArrayList<PVector> {
